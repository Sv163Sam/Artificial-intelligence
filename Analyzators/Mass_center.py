...
def translate_coordinates_to_centers(poses_path: str, centers_path: str, translated_output_path: str, video_base_size: int):
    """
    Преобразует детектированные координаты в координаты относительно центра масс.

    Args:
        poses_path (str): Путь к файлу с координатами.
        centers_path (str): Путь к файлу с центром масс.
        translated_output_path (str): Путь к результирующему файлу.
        video_base_size (str): Количество видеофрагментов.
    Returns:
          None.
    """
    # Убедимся, что выходная папка существует
    os.makedirs(translated_output_path, exist_ok=True)
    ...
    for i in range(1, video_base_size + 1):
        ...
        translated_output_filename = os.path.join(translated_output_path, f"{i}.txt")

        with open(input_filename, 'r') as poses, open(center_filename, 'r') as centers, open(translated_output_filename, 'w') as output:
